...
//#define MAX_COMPONENTS 10
    ...
    if ((colorSpace.equals("DeviceRGB") || colorSpace.equals("RGB")) && bpc == 8) {
        ...
        //bitmap->setConfig(transparencyMask ? SkBitmap::kA8_Config : SkBitmap::kIndex8_Config, width, height);
		bitmap->setInfo(transparencyMask ? SkImageInfo::MakeA8(width, height) : SkImageInfo::Make(width, height, kIndex_8_SkColorType, kPremul_SkAlphaType));
...
*/
            ...
            ((streamDict->isFilterAName(NULL) &&
             ...
             (streamDict->isFilterAArray(NULL) &&
                  ...
                  streamDict->getFilterAsArray(NULL)->objAtAIndex(0)->nameValue2()
                                                                    .equals("DCTDecode")))) {
        SkBitmap* bitmap = new SkBitmap();
		auto data = SkData::MakeFromMalloc(uncompressedStream, uncompressedStreamLength);
		auto codec = SkCodecImageGenerator::NewFromEncodedCodec(data.get());

		if (codec->tryGenerateBitmap(bitmap))
			return bitmap;

		SkPdfReportUnexpectedType(kIgnoreError_SkPdfIssueSeverity, "Cannot decode image", stream,
			SkPdfNativeObject::_kStream_PdfObjectType, pdfContext);
		return NULL;
	}
    ...
    if (indexed) {
		...
		bitmap->setInfo(SkImageInfo::Make(width, height, kIndex_8_SkColorType, kPremul_SkAlphaType));
...
SkPdfRenderer* SkPdfRenderer::CreateFromFile(const char* inputFileName) {
    ...
    SkPdfNativeDoc* pdfDoc = new SkPdfNativeDoc(inputFileName);
    if (pdfDoc->pages() == 0) {
        delete pdfDoc;
        return NULL;
    }

    return new SkPdfRenderer(pdfDoc);
}
...
SkPdfRenderer* SkPdfRenderer::CreateFromStream(SkStream* stream) {
    ...
    SkPdfNativeDoc* pdfDoc = new SkPdfNativeDoc(stream);
    if (pdfDoc->pages() == 0) {
        delete pdfDoc;
        return NULL;
    }
...
bool SkPDFNativeRenderToBitmap(SkStream* stream,
    ...
    SkAutoTUnref<SkBaseDevice> device(new SkBitmapDevice(*output));
